...
// working state or a busy status mean working. This is the same order
// public/roster-panel.js's condition() uses, and deliberately NOT the one
// isWorking uses below: that one answers whether a follow-up may be resumed
// into the session, which is a different question from what to say about it.
...
// mean what the PAGE would paint differently, not what diffRoster noticed:
// diffRoster only fires busy/idle when isWorking flips, and isWorking reads
// `state` before `status`, so a session whose status went busy -> idle under
// a still-"working" state moved as far as the panel is concerned and not at
// all as far as diffRoster was. The panel then showed "working" until some
// unrelated session started or ended. Comparing the projection itself, on
// every successful tick, is what makes the panel follow the machine.
