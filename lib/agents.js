...
// endedAtIn(roster, sessionId) -> that session's stamp, or null when the
// roster has no done record for it.
//
...
export function endedAtIn(roster, sessionId) {
  ...
  return record ? endedAtOf(record) : null;
}
...
// createRosterPoller(opts) -> { start, stop, read, current, fresh, noteEnded }
//
...
export function createRosterPoller(opts = {}) {
  ...
  async function tick() {
    ...
    inFlight = (async () => {
      ...
      // The finish times are kept off the raw listing, before the filter, for
      // the reason trackEnded's comment gives; a failed listing leaves them be.
      if (Array.isArray(next)) ended = trackEnded(ended, next, now());
  ...
  return {
    ...
    // Takes effect on the roster from the next tick; this is not a tick.
    noteEnded(sessionId, at) {
      if (typeof sessionId !== "string" || !sessionId || !Number.isFinite(at)) return;
