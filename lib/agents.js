...
// The wire message, as a string the server can compare tick to tick. The
// roster is sent when it changes rather than on a timer, and "changes" has to
// mean what the PAGE would paint differently -- a status flip under an
// unchanged state (see activity, above), a renumbering, a finish time being
// stamped -- and diffRoster reports only isWorking's edges, so hanging the
// push off its events left the panel saying "working" of an idle process
// until some unrelated session started or ended. Comparing the projection
// itself is what makes the panel follow the machine.
export function rosterWire(roster) {
  return JSON.stringify(rosterForClient(roster));
}
