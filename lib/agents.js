...
// mergeRosters(cli, files) -> one roster, unioned on sessionId.
//
// `claude agents --json` and the ~/.claude/sessions/*.json files it omits
// (lib/session-files.js) are two views of the same set of processes, not two
// rosters -- a hand-started terminal is invisible to one and visible to the
// other, and everything downstream (describeRoster, matchSessions,
// visibleSessions) wants a single list to work from.
//
// The CLI record wins when a sessionId is in both, because it is the richer
// one: it alone carries `state`, the field isWorking and activity() prefer
// over `status`. A CLI record with no pid takes the file record's pid instead
// -- defensively, since every listing observed so far has carried one, and
// the real fix if that ever stops being true is here, not at every call site
// that reads .pid.
export function mergeRosters(cli, files) {
  const cliList = Array.isArray(cli) ? cli : [];
  const fileList = Array.isArray(files) ? files : [];

  const byId = new Map(cliList.map((record) => [record.sessionId, record]));
  for (const record of fileList) {
    ...
    if (existing.pid == null && record.pid != null) {
      byId.set(record.sessionId, { ...existing, pid: record.pid });
    }
  }
  return [...byId.values()];
}
...
function activity(record) {
  ...
  switch (record.status) {
    case "busy":
    // "shell" only ever comes from a file-sourced record (lib/session-files.js)
    // -- a terminal mid shell-command, which isWorking already treats as
    // working. Saying anything else here would have the spoken line disagree
    // with the very bit everything else keys off.
    case "shell":
      return "working";
    case "idle":
      return "idle";
...
// How a single session is referred to out loud. A session named by Dante
...
export function isWorking(record) {
  ...
  // "shell" only ever comes from a file-sourced record -- the CLI listing
  // reports the same session as "busy" via `status`, never this word. A
  // terminal sitting mid shell-command is occupied, and reading it as idle is
  // exactly how the cold `--resume` tell path would fork it instead of
  // joining it.
  return record.status === "busy" || record.status === "shell";
}
...
// listSessions(opts) -> Promise<roster | null>. Never rejects.
//
// The union listAgents alone cannot provide: the CLI half via listAgents (or
// opts.listAgents, the test seam), the state-file half via listSessionFiles
// (or opts.listSessionFiles), read together with Promise.all so one slow side
// does not serialise behind the other.
//
// A null CLI half returns null overall, the same "could not ask" the CLI
// listing already reports on its own -- a files-only roster during a CLI
// outage would read as every --bg session having ended at once, which is
// worse than saying nothing. The files half failing to be an array (the
// injected seam misbehaving; listSessionFiles itself never does) is treated
// as empty rather than propagated, since a foreground terminal going unlisted
// for one tick is a lesser wrong than losing the whole roster over it.
export async function listSessions(opts = {}) {
  const listCli = typeof opts.listAgents === "function" ? opts.listAgents : listAgents;
  const listFiles = typeof opts.listSessionFiles === "function" ? opts.listSessionFiles : listSessionFiles;

  const [cli, files] = await Promise.all([listCli(opts), listFiles(opts)]);
  if (cli === null) return null;
  return mergeRosters(cli, Array.isArray(files) ? files : []);
}
...
export function createRosterPoller(opts = {}) {
  // listSessions rather than listAgents: a hand-started terminal belongs in
  // the standing roster by default, not only when a caller remembers to ask
  // for it. Every test here passes its own `list`, so this default is only
  // ever exercised by the real server.
  const list = typeof opts.list === "function" ? opts.list : listSessions;
  const intervalMs =
