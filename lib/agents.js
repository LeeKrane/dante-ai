...
//
// How many of the sessions on the roster jarvis itself started, and the name of
// the oldest of those that is idle.
//
// The count exists to bound how many unattended sessions jarvis has running
...
// from one jarvis started, so a machine in ordinary use sat at four of five
// before jarvis had done anything, every start was refused, and the refusal
...
// jarvis started that has since died does not either.
export function ownRunning(roster, remembered) {
  ...
  // "constructor" must not report every session as one of jarvis's own.
