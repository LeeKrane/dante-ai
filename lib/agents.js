...
// carryEnded(previous, next, now) -> next, with `endedAt` on every done record
//
...
// otherwise. The tick that first sees a record done is the closest thing to a
// finish time this side of the daemon has, so that tick's clock is stamped on
// it here and carried forward, unchanged, on every later tick.
//
// A record that is not done never carries one: a done session that is resumed
// (Stage 27 joins an idle session) is back on a live clock, and finishing again
// stamps it afresh. A session already done on the first tick after a restart
// gets that tick's time, which is later than the truth. It is still a constant,
// which is the one property a clock on a finished thing must have; the real
// end is not recorded anywhere on the machine.
export function carryEnded(previous, next, now = Date.now()) {
  ...
  return next.map((record) => {
    ...
    // A live record is handed back as it came: `next` is a fresh listing, and
    // nothing upstream of here ever writes an endedAt, so there is none to
    // strip -- a resumed session simply arrives without one.
    if (record.state !== "done") return record;
    const was = before.get(record.sessionId);
    const kept = was && was.state === "done" && Number.isFinite(was.endedAt) ? was.endedAt : null;
    return { ...record, endedAt: kept ?? now };
  });
}

// endedAtOf(roster, sessionId) -> the stamp carryEnded put on that session,
// or null when the roster has no done record for it.
//
// For the hook path. Stop and SessionEnd both report a completion, and they
// are not the same moment: Stop fires when the task is done, SessionEnd when
// the process goes, which can be an hour of sitting idle later. The roster
// kept between has the moment the poller saw it done, so a SessionEnd-driven
// report asks here first and falls back to now only when nothing was seen --
// which is Stop's case, since it lands ahead of the tick that would stamp it.
export function endedAtOf(roster, sessionId) {
  if (!Array.isArray(roster) || typeof sessionId !== "string" || !sessionId) return null;
  const record = roster.find((entry) => isPlainObject(entry) && entry.sessionId === sessionId);
  return record && record.state === "done" && Number.isFinite(record.endedAt) ? record.endedAt : null;
}
...
// stamp when there is one and `now` when there is not; the rule the panel's
// clock and the spoken "took four minutes" both follow, written once.
  ...
  async function tick() {
    ...
    inFlight = (async () => {
      ...
      if (Array.isArray(next)) {
        ...
        // and the stamp has to survive onto them from the roster kept here.
        next = carryEnded(roster, next, now());
