...
// What the session is doing, in one word someone would actually say. An
// unrecognised value becomes "running" rather than being read aloud verbatim:
// a future CLI value would otherwise arrive in the user's ear as jargon.
//
// `state` and `status` are not one field spelled twice. `state` is where the
// TASK stands -- it is "working" from the first turn until the session marks
// itself done or blocked, and a turn that ends without either leaves it there
// -- while `status` is what the PROCESS is doing this instant (busy, idle,
// waiting). A live listing had a session at `state: "working"` and
// `status: "idle"` for hours, the one stopped mid-task and respawned, and
// reading `state` first said "working" of a process doing nothing. So "done"
// and "blocked" are believed from `state` (status has no word for either), an
// idle `status` is believed over a working `state`, and only then does a
// working state or a busy status mean working. public/roster-panel.js's
// condition() applies the same order (public/ cannot import from lib/), pinned
// to this one by test/roster-panel.test.js; the two differ only in the
// fallback, "running" here and "idle" there, since the page has no style for
// a word it has never seen. Deliberately NOT the order isWorking uses below:
// that one answers whether a follow-up may be resumed into the session, which
// is a different question from what to say about it.
export function activity(record) {
...
// rosterForClient(roster) -> the rows the page paints.
//
// Only what a row needs -- no pid, no path, no session id beyond the one that
// keys the row -- because everything sent here is written by whoever started
// the session and lands in a browser. Cut at MAX_LISTED so the message stays
// small whatever a machine running twenty sessions does, and MAX_LISTED rather
// than a number of its own because the numbering is only meaningful if the
// panel and the model are looking at the same cut of the same list. The
// roster arrives already numbered (orderRoster, over the FULL list), so a
// hidden sixteenth session still has a number and "stop session sixteen"
// still resolves even though the panel never draws that row.
//
// server.js compares these rows tick to tick to decide whether the page is
// pushed at all (broadcastRoster), so what a row carries is also what counts
// as a change: a status that moved under an unchanged state is one, a
// renumbering is one, a finish time being stamped is one -- none of which
// diffRoster reports, since its events are isWorking's edges and no more.
export function rosterForClient(roster) {
  ...
  return roster.slice(0, MAX_LISTED).map((record) => ({
    ...
    // that moment rather than counting on from startedAt (see trackEnded,
    // below). null for a live one: its age is counted on the page.
...
// The one reading of "this record is sitting on a permission prompt right
// now", shared so lib/watch.js's three hand-rolled `record.state === "blocked"`
// checks (refuseWatch, tick, resumedAmong) cannot drift from one another or
// from isWorking's own idea of blocked above.
export function isBlocked(record) {
  return isPlainObject(record) && record.state === "blocked";
}
