...
// A session name is short and normalizeName's 100-char clip (inherited from
// cleanLabel) is generous for one, but the haystack mentionedSessions
// searches is a whole turn's words, unanswered sentences included -- a
// rambling opening clause can easily push the actual name past char 100,
// and normalizeName would silently clip it off before the search ever ran.
// This applies the identical reduction (whitespace collapsed, the same
// UNPRINTABLE class stripped, lowercased, non-alphanumerics folded to one
// dash) but caps only at a ceiling generous enough that no turn anyone
// actually speaks reaches it.
...
function normalizeHaystack(value) {
  ...
  return value
    ...
    .replace(UNPRINTABLE, "")
    .trim()
    .slice(0, MAX_HAYSTACK_CHARS)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
}

// mentionedSessions(text, candidates) -> the names of `candidates` whose
// normalized name appears in the normalized `text` as a whole token run,
// bounded by "-" on both sides once both strings are wrapped in one. That
// bounding is what keeps a mention of "jarvis-3" from also matching a
// candidate named "jarvis-30" -- the reduction folds both to a dash-joined
// run of words, and an unbounded substring check cannot tell "-jarvis-3-" (a
// whole name) from the first half of "-jarvis-30-" (someone else's). Used to
// pin whichever session's note a turn actually named ahead of one merely
// folded because it happened to be touched more recently (lib/notes.js's
// pickNotes).
export function mentionedSessions(text, candidates) {
  const said = `-${normalizeHaystack(text)}-`;
  if (said === "--") return [];
  ...
  for (const record of list) {
    ...
    if (!norm) continue;
    if (said.includes(`-${norm}-`)) found.add(record.name);
  }
  return [...found];
}
