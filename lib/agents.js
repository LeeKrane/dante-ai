...
// normalized name appears in normalized `text` as a whole token run, bounded
// by "-" on both sides once both strings are wrapped in one. That bounding
// is what keeps a mention of "jarvis-3" from also matching a candidate named
// "jarvis-30" -- normalizeName would reduce both to a dash-joined run of
// words, and an unbounded substring check cannot tell "-jarvis-3-" (a whole
// name) from the first half of "-jarvis-30-" (someone else's). Used to pin
// whichever session's note a turn actually named ahead of one merely folded
// because it happened to be touched more recently (lib/notes.js's
// pickNotes).
