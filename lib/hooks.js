...
// seconds and works for sessions started before jarvis existed. Hooks are the
...
// is therefore not a nicety here; it is the difference between one thread and
...
// (five seconds) later, plus whatever Slack takes. Thirty seconds covers all
// three reporting one exit. It is deliberately not longer -- a session that
// genuinely finishes twice, because something was queued for it, is news both
// times.
...
// parseHookEvent(payload) -> { kind, sessionId, cwd, detail } | null
//
// null for anything unrecognised, and deliberately without an error: any local
// process can post here, and a loud failure is a way to make jarvis say things.
...
export function parseHookEvent(payload) {
  ...
  // unattributable event is one jarvis has no business reporting.
  ...
  return {
    ...
    // it ended. Both are written by something other than jarvis.
