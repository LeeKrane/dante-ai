...
// paragraphAfter below strips that blank line back off before running the
// boundary searches, which is what makes an empty section still read as
// empty (see its own comment for why the two are not the same fix).
//
// "first" carries `(?![\w-])`, a negative lookahead rather than a plain `\b`:
// a heading can run straight into more letters or a hyphen with nothing
// between them ("Do This First-Draft Notes", "Do This Firstly"), and neither
// is this heading -- `\b` alone does not reject a hyphen, since a hyphen is
// already a non-word character and so already "on the boundary" as far as
// `\b` is concerned.
...
//
// The closing side treats a bare "." as a boundary only when nothing that
// looks like a word follows it: "_important_." ends a sentence, but
// "_config_.json" and "__init__.py" have a file extension on the other side
// of that dot, not the rest of a sentence, and stripping the markers there
// would rewrite a real filename. DOUBLE_UNDERSCORE is given the exact same
// opening and closing rules as SINGLE_UNDERSCORE for that reason -- it used
// to be a bare `/__([^_]+)__/g` with no boundary check at all, which is what
// let "__init__.py" get read as emphasis around "init" in the first place.
const EMPHASIS_CLOSE = "(?=$|[\\s,;:!?)]|\\.(?!\\w))";
const SINGLE_STAR = /(^|[\s(])\*(\S(?:[^*\n]*\S)?)\*(?=$|[\s.,;:!?)])/g;
const SINGLE_UNDERSCORE = new RegExp(`(^|[\\s(])_(\\S(?:[^_\\n]*\\S)?)_${EMPHASIS_CLOSE}`, "g");
const DOUBLE_UNDERSCORE = new RegExp(`(^|[\\s(])__(\\S(?:[^_\\n]*\\S)?)__${EMPHASIS_CLOSE}`, "g");

function stripEmphasis(text) {
  return text
    .replace(/\*\*([^*]+)\*\*/g, "$1")
    .replace(DOUBLE_UNDERSCORE, "$1$2")
    .replace(SINGLE_STAR, "$1$2")
    .replace(SINGLE_UNDERSCORE, "$1$2");
}

// paragraphAfter(text, match) -> the cleaned paragraph following one
// DO_THIS_FIRST_HEADING match, or "" when that heading's section is empty.
// Split out of extractDoThisFirst so that function can ask it of more than
// one heading match -- see the comment there for why the last one is not
// always the right one.
function paragraphAfter(text, match) {
  let after = text.slice(match.index + match[0].length);
  // Markdown convention puts a blank line between a heading and the prose
  // under it; skipped here, before the boundary searches below ever run, so
  // that ordinary blank line is not mistaken for NEXT_BLANK_LINE's own
  // empty-section signal. An EMPTY section is not "a blank line", it is "a
  // blank line with nothing but the next heading behind it" -- stripping the
  // leading blank line first and then asking NEXT_HEADING/NEXT_BLANK_LINE
  // where the paragraph ends is what tells the two apart, because a real
  // paragraph still has non-blank text sitting right after the strip.
...
export function extractDoThisFirst(text) {
  ...
  // Every heading, not just the first -- walked newest-to-oldest, and the
  // first one whose OWN section is non-empty wins. Taking the textually last
  // match unconditionally used to lose a real verdict to a later empty
  // repeat of the same heading (a recap or a table-of-contents echo, say):
  // that heading matches too, and an empty section is a legitimate answer
  // for the match it belongs to, but it must not shadow an earlier section
  // that actually said something.
  const headings = [...text.matchAll(DO_THIS_FIRST_HEADING)];
  for (let i = headings.length - 1; i >= 0; i -= 1) {
    const paragraph = paragraphAfter(text, headings[i]);
