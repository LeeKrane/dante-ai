...
// Unlike loadFishConfig, missing environment variables are not an error. Slack
// is an enhancement: someone who never sets DANTE_SLACK_TOKEN and
// DANTE_SLACK_CHANNEL should still get a working assistant, not a server that
// refuses to start.
export function loadSlackConfig(env = process.env) {
  const botToken = String(env.DANTE_SLACK_TOKEN || "");
  const channel = String(env.DANTE_SLACK_CHANNEL || "");
