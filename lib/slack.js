...
export const DEFAULT_CONFIG_PATH = join(homedir(), ".config", "jarvis", "slack.json");
export const API_URL = "https://slack.com/api/chat.postMessage";
...
export function loadSlackConfig(path = DEFAULT_CONFIG_PATH, env = process.env) {
  ...
  const botToken = String(env.JARVIS_SLACK_TOKEN || cfg.botToken || "");
  const channel = String(env.JARVIS_SLACK_CHANNEL || cfg.channel || "");
