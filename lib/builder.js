...
function deniedFiles(home, repo) {
  return [
    ...
    join(repo, ".env"), // the server's own credentials file: Fish key, Supabase anon key, Slack token
  ];
}
...
export function denyRules(opts = {}) {
  ...
  // private key it read is a private key that can leave the machine. That
  // includes the repo's own `.env` alongside `~/.claude.json`.
  ...
  rules.push(`Read(${rulePath(join(repo, ".env"))})`);
