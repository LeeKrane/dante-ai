...
async function writeBuildSettings(opts) {
  const dir = await mkdtemp(join(tmpdir(), "jarvis-build-"));
...
export async function run(primitive, params, onProgress, opts = {}) {
  ...
  const cleanUpSettings = () =>
    generated ? rm(generated.dir, { recursive: true, force: true }).catch(() => {}) : Promise.resolve();

  const args = buildSpawnArgs(primitive, params, {
    ...opts,
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    // Opened after the spawn so a missing CLI does not leave an empty log behind.
    ...
    child.on("error", (err) => {
      ...
      clearTimers();
      // e.g. ENOENT: the CLI is not installed.
      Promise.all([closeLog(logStream), cleanUpSettings()]).then(() => reject(err));
    });
    ...
    child.on("close", async (code) => {
      ...
      await closeLog(logStream); // resolve only once the log is complete on disk
