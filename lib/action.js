...
//
// An unquoted value runs to the next key= boundary, not the next space. The
// persona tells the model to copy a session name off the machine-state lines
// "exactly as written", and a model told that does not go add quotes around it
// - so a hand-named session ("slack integration setup") or a repo-qualified
// label describeRoster produces ("repo-persistence: jarvis-10-...") has spaces
// in it, and stopping at the first one silently truncated a real session into
// one that "does not exist" (see lib/agents.js's aliasFor and label).
//
// The unquoted alternative advances a whole token at a time - \S+, then zero
// or more (\s+ token) repetitions - rather than a per-character lookahead, so
// the "is the next token a key=" check runs once per token boundary and the
// scan is linear in the body's length. An earlier version checked that
// lookahead at every character instead of every token boundary and looked
// linear on the tests here, which all run over short bodies, but it was
// actually O(k^2) over a long run of whitespace inside one value (measured:
// 8000 spaces, 108ms) - the per-character version is not in this file any
// more, precisely because that claim did not hold for it.
const PAIR =
  /(?:^|\s)([^\s=]+)\s*=\s*("[^"]*"|'[^']*'|(?:\S+(?:\s+(?![^\s=]+\s*=)\S+)*)?)/g;

// Strip one matched pair of surrounding quotes. A lone quote is left alone - it is
// more likely part of the value than a broken quoting attempt. No trim is needed
// on the unquoted branch: PAIR's unquoted alternative is built from whole
// (whitespace, token) repetitions, so a match can only ever end on a token - it
// is structurally unable to capture the trailing whitespace leading into the
// closing "]" or the next key. Verified directly against
// "[ACTION:BUILD primitive=landing-page ]" and a name= holding three trailing
// spaces before "]": neither leaves a trailing space in `value`.
