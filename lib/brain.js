...
function sessionsBlock(kinds) {
  ...
  lines.push(
    ...
    "The name is the one from the running-sessions line in the turn - use it exactly as written",
    "there, never a number on its own, never a position in the list, and never a description.",
    "If Jesse names a session you cannot see in that line, say so rather than guessing at one.",
    "NEVER emit verb=tell or verb=stop unless Jesse asked you, in this turn, to say something to",
    "that session or to stop it. A session appearing in the running-sessions line is something",
    ...
    "A tag is a PROPOSAL, not an act. Jesse is asked to confirm it before anything runs, so say",
    "what you are about to do, never that it is done, and never guess at a repository, a task or",
    "which session he means in order to have something to tag - ask him instead.",
    "Say one short sentence and nothing else: you will not know how it went for",
    "minutes, so do not promise a result. Like every other tag it is stripped before you are",
    "heard: never mention it, never read it aloud.",
