...
function sessionsBlock(kinds) {
  ...
  lines.push(
    ...
    'verb=tell number="<n>" task="what to tell it" when Jesse named the session by its number',
    "instead of its name.",
    ...
    "at a name whenever Jesse addressed the session that way, and once you know which one you",
    'acted on, acknowledge it back the same way he said it: "Session three." is enough on its own,',
    "or the number and the name together when there is room for both.",
    ...
    'number="<n>" in place of name= when Jesse addressed a still-running session by its number',
    ...
    "(constraints), and what done looks like (acceptance) - interview Jesse first: one question",
    "per reply, most important first, and nothing else in that reply - Jesse answers by voice",
    "and cannot answer three questions at once. Append",
    '[ACTION:SESSION verb=interview for=<start|tell|interrupt> repo=<alias> have=<facet,facet>',
    'note="what his last answer taught you"] to every such question. have is the facets you are',
    "now sure of, drawn from goal, where, constraints, done - written with no spaces, comma-",
    'separated, like have=goal,constraints, or in double quotes if you must write it with spaces.',
    "note is one sentence capturing everything the last answer told you, in his own terms - not a",
    "paraphrase that drops a detail, because the note is what survives if you are restarted.",
    ...
    "failing), or for confirmation of something you could state as an assumption. Skip the",
    "interview when the request is already specific or small enough to brief in one line, and",
    "stop at once when Jesse says to just go or to stop asking - then propose with what you have.",
    ...
    "every detail from the interview, in Jesse's own terms, nothing summarised away and nothing",
