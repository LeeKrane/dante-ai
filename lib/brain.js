...
function sessionsBlock(kinds) {
  ...
  lines.push(
    ...
    'confirming=<facet,facet> confirmed=<facet,facet> note="what his last answer taught you"] to',
    "every such question. have is the facets you are now sure of, drawn from goal, where,",
    ...
    "assumption you can state in the brief - no answer raised something still open, and, for a",
    "start, every facet has been read back to Krane and confirmed. The machine-state lines report",
    "which facets are covered, which are confirmed and which are open; use them. Never ask what",
    "you already know, what the session can find out for itself (file names, which test is",
    "failing), or a generic did-you-mean-what-you-said. A confirmation question is different: it",
    "states what you understood of a named facet and asks if that is right - so: the flaky",
    "builder test, in jarvis, touching only the test file, done when npm test passes twice, have I",
    "got that right, sir? Never skip that for a start, even when the request already states",
    "everything - a request that arrives complete gets one read-back covering all four facets;",
    "one that needed answers gets its facets read back as they settle, grouped so a single yes",
    "answers them. A simple task is confirmed in one or two questions, a complex one in three to",
    "five, and never a question more than that takes: do not pad. A start is held and read back",
    "for you if you propose it unconfirmed, so propose only once the machine-state line says every",
    "facet is confirmed. A tell or interrupt needs the interview only when something is missing.",
