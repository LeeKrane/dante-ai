...
export function stopVerdict({ name, result, listed } = {}) {
  const who = typeof name === "string" && name ? name : "that session";
...
// startVerdict({ name, listed }) -> spoken
//
...
export function startVerdict({ name, listed } = {}) {
  const who = typeof name === "string" && name ? name : "that session";
  if (listed === true) return `Running as ${who}.`;
  if (listed === null) return `Started as ${who}, sir, but I could not check the roster.`;
  return `Started as ${who}, sir. It is not on the roster yet.`;
}
