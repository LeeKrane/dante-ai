...
export function rememberEnded(store, roster) {
  ...
  for (const record of roster) {
    if (!isPlainObject(record) || record.state !== "done" || !Number.isFinite(record.endedAt)) continue;
    const remembered = getSessionRecord(store, record.sessionId);
    if (!remembered || remembered.endedAt === record.endedAt) continue;
    rememberSession(store, record.sessionId, { endedAt: record.endedAt });
    changed += 1;
  }
  return changed;
}
