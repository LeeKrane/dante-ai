...
export const MAX_ALIAS_CHARS = 40; // an alias is a word someone says out loud
...
// Sessions jarvis started, kept so a name can be resolved back to what it was
...
export function emptyStore() {
  return { version: 1, projects: {}, workspaces: {} };
}
...
// ---------------------------------------------------------------------------
//
// A cwd is not a thing anyone says aloud. A workspace is one alias — "jarvis",
// "fitness" — bound to one absolute directory, plus the counter that makes the
// session names jarvis-1, jarvis-2, jarvis-3 unique without consulting
// anything else.
//
...
// stored records, which also carry a counter that describeRoster has no use for.
...
export function addWorkspace(store, path, alias, opts = {}) {
  ...
  // An explicit alias for a path already known renames nothing: the counter is
  // bound to the alias, and moving it would restart session numbering and
  // orphan every name already spoken.
  ...
  store.workspaces[key] = { path: real, counter: 0, addedAt: new Date().toISOString() };
  return { alias: key, ...store.workspaces[key] };
}

// Mutates the workspace's counter and returns the number the next session in
// that repository should carry. Counting per workspace rather than globally is
// what makes "jarvis one" and "fitness one" both sayable, and both unambiguous.
export function nextSessionNumber(store, alias) {
  const key = sanitizeAlias(alias);
  const entry = getWorkspaces(store)[key];
  if (!isPlainObject(entry)) return null;

  const current = Number.isInteger(entry.counter) && entry.counter >= 0 ? entry.counter : 0;
  entry.counter = current + 1;
  return entry.counter;
}

// The seam for [MEMORY:SET workspace:fitness=/home/you/dev/KraneticFitness].
// Reads the workspace-prefixed pairs out of an already-parsed tag bag, leaving
// the rest for applyMemoryTag. Returns what was actually registered, or null —
// so the caller can stay quiet rather than narrate a path it refused.
...
// it on the first tick that sees the session idle.
...
export function queueForSession(store, sessionId, text, now = Date.now()) {
  ...
  const line = cleanText(text, MAX_QUEUED_CHARS);
...
// it, so a later patch (a Slack thread id, an outcome) cannot wipe what the
...
// stopped from jarvis itself (`stoppedAt` set on its remembered record) drops
...
// kind is not one of lib/notify.js's KINDS -- the same set Slack's own
// formatEvent refuses to post, so a garbled kind never gets further than either
// half of the notification.
