...
//
// lib/notes.js is the second half of memory: many human-readable files on
// disk instead of this one JSON document, weighted and pruned rather than
// capped by key count, and the limits that govern its pruning live in this
// store under `noteLimits` (applyNoteLimitsTag, getNoteLimits, below).
...
import { endedAtOf } from "./agents.js";
import { MAX_BRIEF_CHARS, cleanBrief } from "./interview.js";
import { MAX_DO_THIS_FIRST_CHARS } from "./transcript.js";
// Only for the two defaults: lib/notes.js must never import this file back
// (it would be a cycle for no reason either side needs), but this direction
// is fine -- the limits this file stores are about lib/notes.js's directory,
// so its defaults are the one source of truth for what "no limit set" means.
import { DEFAULT_MAX_BYTES as NOTES_DEFAULT_MAX_BYTES, DEFAULT_MAX_FILES as NOTES_DEFAULT_MAX_FILES } from "./notes.js";
...
// [MEMORY:SET memory-max-mb=100] and [MEMORY:SET memory-max-files=200] set
// how much of lib/notes.js's own note directory to keep before the oldest
// notes are pruned. Same reasoning as WORKSPACE_PREFIX and MAIN_KEY above:
// these are settings, not standing prose, and left in sanitizePreferences
// they would be folded into every future prompt as if Krane had said "memory
// max mb is 100" out loud.
export const NOTE_LIMIT_KEYS = { maxBytes: "memory-max-mb", maxFiles: "memory-max-files" };
...
export function sanitizePreferences(bag) {
  ...
  for (const rawKey of Object.keys(bag)) {
    ...
    // Same reasoning again: applyNoteLimitsTag owns both of these keys, and
    // left here either would be stored as standing prose ("memory max mb is
    // 100") instead of ever reaching store.noteLimits.
    if (key === NOTE_LIMIT_KEYS.maxBytes || key === NOTE_LIMIT_KEYS.maxFiles) continue;
...
// A range check local to this store, deliberately not lib/notes.js's own
// sanitizeLimits: that function's ceiling is a security boundary for the
// notes directory itself and is expressed in bytes, while this one is a
// convenience for a spoken "keep up to 100 megabytes" -- validating the
// megabyte figure here, before ever multiplying it out to bytes, is what
// keeps a wildly out-of-range spoken number from being convertible into a
// byte count sanitizeLimits would then have to clamp back down anyway.
function inRange(n, min, max) {
  return Number.isFinite(n) && n >= min && n <= max;
}

// applyNoteLimitsTag(store, memory) -> { maxBytes, maxFiles } | null. Reads
// NOTE_LIMIT_KEYS out of the raw tag bag (not the already-sanitized one --
// sanitizePreferences strips these two keys on purpose, same as it does
// workspace: and main), parses them as megabytes/file-count integers, and
// stores whichever validate under store.noteLimits, merging with whatever is
// already there so setting one key never clobbers the other. Returns what
// changed, or null when nothing valid was present.
export function applyNoteLimitsTag(store, memory) {
  if (!memory || typeof memory !== "object") return null;

  const patch = {};

  const rawMb = memory[NOTE_LIMIT_KEYS.maxBytes];
  if (rawMb !== undefined) {
    const mb = Number(stripUnprintable(rawMb).trim());
    if (inRange(mb, 1, 2048)) patch.maxBytes = Math.trunc(mb) * 1024 * 1024;
  }

  const rawFiles = memory[NOTE_LIMIT_KEYS.maxFiles];
  if (rawFiles !== undefined) {
    const files = Number(stripUnprintable(rawFiles).trim());
    if (inRange(files, 1, 100000)) patch.maxFiles = Math.trunc(files);
  }

  if (Object.keys(patch).length === 0) return null;

  store.noteLimits = { ...(isPlainObject(store.noteLimits) ? store.noteLimits : {}), ...patch };
  return patch;
}

// getNoteLimits(store) -> { maxBytes, maxFiles }, defaulting to lib/notes.js's
// own defaults when store.noteLimits is absent or hand-edited into something
// that is not a plain object -- a malformed limits field must cost the notes
// feature its custom limit for that read, never the whole store.
export function getNoteLimits(store) {
  const raw = isPlainObject(store?.noteLimits) ? store.noteLimits : {};
  const maxBytes = Number.isFinite(raw.maxBytes) && raw.maxBytes >= 1 ? raw.maxBytes : NOTES_DEFAULT_MAX_BYTES;
  const maxFiles = Number.isFinite(raw.maxFiles) && raw.maxFiles >= 1 ? raw.maxFiles : NOTES_DEFAULT_MAX_FILES;
...
// repoLetter(index) -> "A".."Z" for 0..25, then "AA", "AB"... beyond, the same
// bijective base-26 scheme a spreadsheet uses for column headers (there is no
// zero digit, which is why the divide step subtracts one before it recurses).
// MAX_WORKSPACES is 20, so only A..T are ever assigned in practice; the
// two-letter tail exists only so a hand-edited memory.json with more entries
// than the cap allows degrades to more letters instead of colliding on one.
// A non-integer or negative index is not a position in the list at all
// (NaN, a float, someone passing -1) and gets "" rather than a guess.
export function repoLetter(index) {
...
// { alias, main, letter } for every workspace, main first and the rest in
// alphabetical alias order -- the same order the session panel groups its
// rows in, so "here is what is running" and "here is where it can run" read
// the same way. `letter` is assigned fresh in that already-sorted order on
// every call rather than stored, exactly like a session's number: it is a
// fact about this view, not about the workspace, and recomputing it here is
// what keeps A pinned to the main repository whenever main changes rather
// than to whichever workspace happened to be first historically.
...
export function workspacesForClient(store) {
  ...
  return entries.map((entry, i) => ({ ...entry, letter: repoLetter(i) }));
}

// resolveRepoRef(store, value) -> the alias `value` actually names, or `value`
// unchanged when it names nothing this store knows. Lets "repo B" reach every
// call site that already expects an alias -- confirmation sentences, the
// activity line, findTarget's own alias check -- without any of them having to
// know letters exist.
//
// A real alias always wins over a letter (checked first): someone may have
// registered a workspace literally called "b", and a letter is only ever a
// spoken shorthand for whichever repository happens to sort into that slot on
// this tick, never a name anyone actually gave a repository. Only after that
// miss is `value` tried as a one- or two-letter position against the current
// workspacesForClient order (case-insensitively -- a model may write it either
// way). Anything else -- a longer word, a name matching no workspace and no
// letter -- passes through unchanged, so refuseStart still gets to say "I do
// not know a repository called X" about the actual word Krane said rather
// than a letter this function invented.
export function resolveRepoRef(store, value) {
  if (typeof value !== "string") return "";

  const known = getWorkspace(store, value);
  if (known) return known.alias;

  const trimmed = value.trim();
  if (/^[a-z]{1,2}$/i.test(trimmed)) {
    const upper = trimmed.toUpperCase();
    const match = workspacesForClient(store).find((entry) => entry.letter === upper);
...
// rememberEnded(store, roster) -> how many records changed.
//
// The finish time the roster poller stamps on a done session (trackEnded in
// lib/agents.js) lives in the poller's memory, which a restart empties; the
// first tick after would then stamp every session still sitting done with
// restart time, and a clock on a finished thing would move once. Recording
// the stamp on the session's own remembered record is what lets endedSeeds
// hand it back to the poller on the next start. Only sessions Dante started
// are remembered at all, so only those survive a restart; a stranger's done
// session is re-stamped, which is the constant-from-here-on the panel needs
// even if it is later than the truth.
//
// Called on every tick and cheap for it: a write only where the stamp is new
// or moved, and nothing else on the record is touched.
export function rememberEnded(store, roster) {
  if (!Array.isArray(roster)) return 0;
  let changed = 0;
  for (const record of roster) {
    const endedAt = endedAtOf(record);
    if (endedAt === null) continue;
    const remembered = getSessionRecord(store, record.sessionId);
    if (!remembered || remembered.endedAt === endedAt) continue;
    rememberSession(store, record.sessionId, { endedAt });
    changed += 1;
  }
  return changed;
}

// endedSeeds(store) -> [[sessionId, endedAt]], every remembered finish time,
// for the poller to start from. Stale ones cost nothing: the poller keeps a
// seed only while the listing reports that session done (trackEnded).
export function endedSeeds(store) {
  const seeds = [];
  for (const [id, record] of Object.entries(getSessions(store))) {
    if (isPlainObject(record) && Number.isFinite(record.endedAt)) seeds.push([id, record.endedAt]);
  }
  return seeds;
}
...
// Same cap MAX_DO_THIS_FIRST_CHARS enforces in lib/transcript.js and
// lib/notify.js -- server.js's own extraction already capped this once;
// this is the same defensive second cap every other event field gets here.
// Imported rather than re-typed as 240, which is how this and lib/notify.js's
// own copy drifted into existing as three definitions of the same number.
export const MAX_EVENT_DO_THIS_FIRST_CHARS = MAX_DO_THIS_FIRST_CHARS;
...
export function recordEvent(store, entry, now = Date.now()) {
  ...
  const recorded = {
    ...
    // The council's one concrete next step, when reportComplete's own
    // speaksVerdict gate (lib/sessions.js) found one -- carried onto the
    // event so formatEvent's log line, formatRecap's "catch me up" and this
    // log all say the same thing, not only formatSpoken.
    doThisFirst: cleanText(entry.doThisFirst, MAX_EVENT_DO_THIS_FIRST_CHARS),
    at: now,
  };
