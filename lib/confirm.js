// Propose, then act.
//
// Everything jarvis can do to a live process arrives the same way: a model
...
function describeBody({ session, action, primitive, workspace } = {}) {
  if (session) {
    ...
    // The alias jarvis resolved beats the one the model wrote: they can differ,
    ...
    if (verb === "start") {
      ...
      return task ? `Start a session${place} to ${task}.` : `Start a session${place}.`;
    }
    if (verb === "tell") {
