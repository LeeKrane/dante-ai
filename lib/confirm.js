...
import { NO, NO_PHRASES, YES, YES_PHRASES, parseYesNo } from "./approval.js";
import { countWord, matchSessions } from "./agents.js";
import { MAX_COMMAND_CHARS } from "./commands.js";
...
// A repository alias read back inside a sentence. lib/interview.js and the
// session tag parser cap the same field with this same number.
export const MAX_ALIAS_CHARS = 40;

// The five verbs that reach a live process, or a promise about one. verb=read
// is deliberately absent -- see the comment on it inside describeBody below,
// which is the actual reasoning: a read touches nothing, so nothing there
// needs a "Shall I, sir?" in front of it. recap is absent for the same reason
// (and its own comment, further down). watch reaches no process either, but
// it stands up something that persists -- a misheard "watch jarvis-1"
// confirmed unchecked is a promise to report on the wrong session, spoken
// later to whoever is listening then, and the whole point of this gate is
// that nothing standing gets created without a yes. unwatch stays out of this
// set: it dismantles a promise rather than making one and touches no
// process, the same class of thing as read.
const CONFIRMED_VERBS = new Set(["start", "tell", "interrupt", "stop", "watch"]);
...
// partial or oversized match is not a number Krane actually said.
...
// findTarget(roster, query, { number, sessionId, alias }) -> { record,
// refusal }, exactly one of which is non-null. The pure half of server.js's
...
export function findTarget(roster, query, { number, sessionId, alias } = {}) {
  ...
  // something Krane ever says, it is server.js re-targeting the exact process
  ...
  if (number !== undefined && number !== null) {
    ...
    if (record) {
      // Only this path gets the cross-check: a bare number is global and
      // already unique on its own (findTarget just resolved it to exactly
      // one record), so a repo alongside it is not needed to find the
      // session -- it is a second, independent thing Krane said about it, a
      // check on what was heard rather than a lookup key. The sessionId path
      // above is server.js re-targeting a session a proposal already
      // resolved once, not something addressed fresh, so there is nothing
      // new to cross-check there; the name path below already had the name
      // to go on and a mismatched alias would just be the same wrong guess
      // said twice.
      if (
        typeof alias === "string" && alias.trim() &&
        typeof record.alias === "string" && record.alias &&
        record.alias.toLowerCase() !== alias.trim().toLowerCase()
      ) {
        return {
          record: null,
          refusal: `Session ${countWord(wanted)} is in ${record.alias}, not ${alias.trim()}, sir.`,
        };
      }
...
export function clarify({ session, target } = {}) {
  ...
  if (verb === "tell") {
    ...
    const where = cleanText(session?.repo, MAX_ALIAS_CHARS);
...
function describeBody({ session, action, primitive, workspace, target } = {}) {
  if (session) {
    ...
    const where = cleanText(workspace?.alias ?? session.repo, MAX_ALIAS_CHARS);
    ...
    // A slash command is said back exactly, arguments and all, because the
    // rule of this module -- say back exactly what will run -- has never
    // applied more literally: the line spoken here is the line the session
    // receives. It is not run through asClause, which would drop its
    // punctuation. Only a line that still starts with its slash counts; the
    // vetting is lib/commands.js's job and this only refuses the shape.
    const commandLine = cleanText(session.command, MAX_COMMAND_CHARS);
    const command = commandLine.startsWith("/") ? commandLine : "";

    if (verb === "start") {
      const place = where ? ` in ${where}` : "";
      if (command) return `Start a session${place} running ${command}.`;
    ...
    if (verb === "tell") {
      ...
      if (!who) return null;
      if (command) return `Send ${command} to ${who}.`;
      return task ? `Tell ${who} to ${task}${withBrief}.` : null;
    }
    if (verb === "interrupt") {
      const who = whoFor(session, { resolved, name, where });
      if (!who) return null;
      // No command clause here: a slash command cannot cut in front of a
      // turn (see dispatchTell in server.js), so server.js turns a command
      // interrupt into a command tell before it is ever described.
    ...
    if (verb === "watch") {
      const who = whoFor(session, { resolved, name, where });
      return who ? `Watch ${who} and tell you the moment it stops working.` : null;
    }
...
// A correction to a read-back tends to open with an agreement -- "yes, but
// only the test file" -- because the person is confirming most of what they
// hear and flagging the one part that's wrong, not refusing the question
// outright. readAnswer's four-word cutoff reads that as "amend" already
// (five words), but a longer sentence built the same way -- "yes, that's
// exactly right, go ahead and start it" -- says nothing that needs
// correcting and would still fail the word count for no reason that matters
// here.
//
// Filler words that show up in a real yes and carry no content of their own
// -- "that's exactly right", "nothing to change" -- once a YES word or
// phrase is already present. This is deliberately an allowlist rather than a
// blacklist of words that introduce a correction: a blacklist can never
// enumerate every noun a correction might carry ("sure, in the fitness
// repo", "okay so make it the whole test suite" -- "fitness" and "suite"
// are not correction markers, they ARE the correction), so any word this
// utterance contains that is neither a YES word nor filler is exactly where
// a correction hides, and reads as one. Erring toward "amend" costs one
// extra read-back; erring toward "yes" sends an uncorrected brief on to a
// real session, which is the more expensive way to be wrong.
//
// The known gap this leaves: a NO-vocabulary word inside a genuine yes --
// "yes that's right, don't stop now" -- still reads as amend, because hasNo
// below does not know the "don't" is aimed at something other than the
// read-back itself.
const AFFIRMATION_FILLER = new Set([
  "that", "that's", "thats", "is", "it", "it's", "its", "exactly", "right",
  "correct", "perfect", "fine", "good", "great", "all", "of", "sir", "please",
  "thanks", "thank", "you", "then", "start", "now", "so", "the", "a", "this",
  "one", "spot", "on", "sounds", "looks", "absolutely", "indeed", "precisely",
  "nothing", "to", "change", "actually", "got", "have", "i", "think", "yep",
  "yeah", "go", "ahead", "do", "for", "let's", "lets",
]);

// readConfirmingAnswer(text) -> "yes" | "no" | "amend"
//
// The vocabulary readAnswer uses for a proposal's "Shall I, sir?" is too
// strict for this read-back: there, a longer yes costs nothing but a
// re-proposal that gets confirmed again, so the asymmetry favours treating
// anything long as a correction. Here, a yes runs nothing -- it only lets
// the held tag through to proposeSession's own "Shall I, sir?", which Krane
// can still decline -- so misreading a long, genuine yes as a correction is
// the expensive direction: the model re-proposes, and the whole read-back
// gets spoken a second time, which is the duplicate this module exists to
// stop. So past readAnswer's word cutoff, this reads "yes" out of a longer
// sentence only when every word in it is accounted for: a YES word or
// phrase is present, no NO word or phrase is, and every remaining word is
// filler. Anything else is "amend".
export function readConfirmingAnswer(text) {
  const words = cleanText(text, 300).split(/\s+/).filter(Boolean);
  if (words.length <= MAX_ANSWER_WORDS) return readAnswer(text);

  const normalized = cleanText(text, 300)
    .toLowerCase()
    // Apostrophes survive so "that's" and "don't" stay one word, same as
    // parseYesNo's own normalization.
    .replace(/[^a-z0-9'\s]/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  const normalizedWords = normalized ? normalized.split(" ") : [];

  const hasYes = normalizedWords.some((word) => YES.has(word)) || YES_PHRASES.some((p) => p.test(normalized));
  const hasNo = normalizedWords.some((word) => NO.has(word)) || NO_PHRASES.some((p) => p.test(normalized));
  const allFiller = normalizedWords.every((word) => YES.has(word) || AFFIRMATION_FILLER.has(word));

  return hasYes && !hasNo && allFiller ? "yes" : "amend";
}
