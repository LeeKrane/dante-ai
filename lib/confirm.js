...
// parseSessionNumber(value) -> a positive integer, or null.
//
...
// negative number, "3a", a float -- is refused: zero and negative are not
// positions on a 1-based list, and a partial match is not a number Jesse
// actually said.
...
// findTarget(roster, query, { number }) -> { record, refusal }, exactly one of
// which is non-null. The pure half of server.js's resolveSession: matching a
// spoken name (or a numbered line) against the roster and choosing the
// sentence for each outcome is logic, not wiring, and belongs on this side of
// the line where it can be tested without a live process behind it.
//
// The roster-readable check moves above both paths, number and name: a
// listing that failed is "I cannot see what is running" whichever way the
// session was addressed, not just when it was addressed by name.
export function findTarget(roster, query, { number } = {}) {
  ...
  const wantedNumber = parseSessionNumber(number);
  if (wantedNumber !== null) {
    const record = roster.find((r) => r.number === wantedNumber);
    if (record) return { record, refusal: null };
    const count = roster.length === 0 ? "none" : countWord(roster.length);
    return {
      record: null,
      refusal: `There is no session ${countWord(wantedNumber)}, sir. I count ${count}.`,
    };
  }
