...
// here.
//
// "and" is not in this set: "yes that is right and start it now" is a plain
// yes said in one breath, and treating "and" as a correction marker turned
// it into "amend" -- which withdraws the hold and makes the read-back get
// spoken a second time, the exact duplicate this function exists to prevent.
// "also" stays, because it still catches the sentence "and" alone cannot --
// "yes and also skip the lint" is a correction, and "also" is the word that
// says so. "not" is not in this set either, but for a different reason: it
// is unreachable. parseYesNo's own NO vocabulary includes "not", so any text
// containing it already fails the `parseYesNo(text) !== "yes"` check above
// and returns "amend" before this set is ever consulted.
//
// A marker list cannot catch every correction, and one residual case is
// worth naming: a correction that carries none of these words at all --
// "yes in the fitness repo please" -- reads as a plain yes. That is
// recoverable rather than a real failure, because the "Shall I, sir?"
// proposal that follows names the repo out loud and can still be declined;
// the read-back's job was never to be the last word, only the common one.
const CORRECTION_MARKERS = new Set([
  "but", "except", "only", "actually", "instead", "also", "rather", "though",
  "although", "however", "change", "plus",
]);
...
// stop. So past readAnswer's word cutoff, this still reads "yes" out of a
// longer sentence, as long as parseYesNo agrees and none of the words that
// tend to introduce a correction show up in it.
export function readConfirmingAnswer(text) {
  const words = cleanText(text, 300).split(/\s+/).filter(Boolean);
  if (words.length <= MAX_ANSWER_WORDS) return readAnswer(text);

  if (parseYesNo(text) !== "yes") return "amend";
  const lower = words.map((word) => word.toLowerCase().replace(/[^a-z']/g, ""));
  return lower.some((word) => CORRECTION_MARKERS.has(word)) ? "amend" : "yes";
}
