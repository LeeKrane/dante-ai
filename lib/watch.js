...
import { countWord, idleAmong, isBlocked, isWorking, matchSessions } from "./agents.js";
...
export function createWatchers() {
  ...
  return {
    // add({ sessionId, name, cwd, task, state, alias, startedAt }) -> boolean.
    // Refuses (returns false, never throws) a sessionId that is not a
    // non-empty string, one already present, or a watch that would push past
    ...
    // still cannot exceed the cap. alias and startedAt are the two fields
    // stored only for a later ghost row (see the comment below); the shape
    // must match what dispatchWatch (server.js) actually passes, since
    ...
    // very tick (server.js's onRoster computes it via watchSkip, below). A
    // watch on one of them has its idle branch alone suppressed -- not fired,
    // and its `watch.state` not advanced either -- because the session is
    // about to go back to work the moment the tell lands, and firing "idle"
    // now would report an ending that never happens. This has to live here,
    // inside tick, rather than as a filter server.js applies to tick's own
    // return value: tick deletes a watch from the registry the instant it
    // decides to fire it, so by the time server.js saw the result there
    // would be nothing left to un-fire.
    //
    // Blocked and gone are NOT suppressed by skip, unlike an earlier version
    // of this function: a permission prompt raised by the very tell being
    // delivered is real news the instant it happens, not something the tell
    // landing will undo, and a session that vanished mid-delivery is exactly
    // as gone whether or not a tell was queued for it. Holding either back
    // for the rest of the drain (up to TELL_TIMEOUT_MS) would leave it
    // unreported for no reason skip exists to serve -- skip is about not
    // reporting an ending that is about to be reversed by the delivery, and
    // neither blocked nor gone ever is.
...
// watchSkip(roster, queuedIds, inFlightIds) -> the Set of sessionIds tick()
// above must leave untouched this tick -- exactly the union tick's own
// comment on `skip` describes: idle sessions with a queued tell about to be
// delivered (idleAmong, lib/agents.js, over `queuedIds`) plus whatever a
// delivery already in flight names (`inFlightIds` -- createInFlight's own
// ids(), lib/spawn-session.js). The union matters on its own: takeQueued
// deletes a queue entry the instant a drain starts, so a session is no
// longer "queued" from the very next tick even though tellSession can still
// be running against it for up to TELL_TIMEOUT_MS -- `inFlightIds` is what
// stays true for the whole drain, not only its first tick. Pulled out here,
// pure, so server.js's onRoster can call this instead of hand-unioning the
// two sets itself.
export function watchSkip(roster, queuedIds, inFlightIds) {
  const skip = new Set(idleAmong(roster, queuedIds).map((record) => record.sessionId));
  for (const id of inFlightIds ?? []) skip.add(id);
  return skip;
}
...
// pruneFired(recentlyFired, roster, now, ghostMs) -> a copy of recentlyFired
// with every entry that deserves forgetting removed. A copy, not a mutation
// -- every other function in this module leaves what it is handed untouched,
// and server.js reassigns its own module-scope binding to the result rather
// than counting on this to mutate the Map in place. Without this the map
// would grow for the entire life of the process: a watch fires at most once,
// and nothing else ever clears its entry back out.
//
// `roster` is checked first: a sessionId still listed there is kept no
// matter how old its firedAt is -- ghostMs is the window a GONE session's
// synthesised row lingers for once nothing on the roster points at it any
// more (ghostRecords, above), not an expiry on the dot a still-listed
// blocked session earns (rosterForClient, server.js). Without this, a
// permission prompt that sits unanswered past ghostMs would have its own
// "reported" mark aged out from under it while the block itself is still
// very much unresolved. Only a sessionId absent from the roster ages out on
// the ordinary ghostMs clock.
export function pruneFired(recentlyFired, roster, now, ghostMs) {
  const liveIds = new Set((Array.isArray(roster) ? roster : []).map((record) => record?.sessionId));
  const pruned = new Map();
  for (const [sessionId, fired] of recentlyFired) {
    if (liveIds.has(sessionId)) {
      pruned.set(sessionId, fired);
      continue;
    }
...
// describeFired({ name, change, state, text, reason, doThisFirst }) -> the spoken report
...
export function describeFired({ name, change, state, text, reason, doThisFirst } = {}) {
  ...
  if (change === "gone") {
    return `${who} has finished, sir. ${said}${tail}`;
  }
  if (change === "blocked") {
    return `${who} is blocked, sir, waiting on a permission prompt. ${said}${tail}`;
  }
  ...
  return `${who} has stopped working, sir${stateClause} ${said}${tail}`;
}
