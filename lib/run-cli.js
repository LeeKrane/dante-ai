...
// child that ignores SIGTERM still holds its stdio pipes in this process, and
// left alone it would hold them until the machine restarts.
...
// `opts.killGraceMs` the gap between the two signals; `opts.maxStdout` and
// `opts.maxStderr` the caps. stdin is always closed rather than inherited: a
// CLI left holding the parent's stdin waits for input that is never coming.
export function runCli(bin, args, opts = {}) {
  const maxStdout = positiveNumber(opts.maxStdout, MAX_STDOUT_BYTES);
  ...
  return new Promise((resolvePromise) => {
    ...
    const timeoutMs = positiveNumber(opts.timeoutMs, null);
    const deadline = timeoutMs === null ? null : setTimeout(() => {
      ...
      }, positiveNumber(opts.killGraceMs, KILL_GRACE_MS));
      hardKill.unref?.();
