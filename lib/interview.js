...
// write one.
//
// Two exact shapes only, not a generic "clause before a colon" match -- an
// earlier version of this matched any short clause ending in a colon, and
// that was wrong two different ways: an option list the model genuinely
// meant to ask ("First interview or full build, sir: which do you want?")
// got its own question eaten as if it were the announcement, and a colon
// or a time sitting inside the announcement itself ("...at 3:15: is now a
// good time...") made the cut land mid-clause instead of at its end. Both
// are corruptions -- a garbled question spoken aloud -- which is worse than
// leaving the announcement in, so the two patterns below match only the
// literal openings the persona names ("before I interview...", "first, a
// question...") plus an optional "sir"/"krane" address, ended by real
// closing punctuation (colon, full stop, semicolon or dash) with nothing
// else in between. Anything that doesn't fit that shape -- a lead-in that
// only mentions the interview in passing, an option list, punctuation
// inside the clause -- is left alone.
const SIR = String.raw`(?:,?\s*(?:sir|krane))?`;
const END = String.raw`\s*[:.;\-–—]+\s*`;
const BEFORE_I_INTERVIEW = new RegExp(
  String.raw`^before\s+(?:i|we)\s+(?:[a-z']+\s+){0,3}interview(?:ing)?(?:\s+you)?${SIR}${END}`,
  "i",
);
const FIRST_A_QUESTION = new RegExp(
  String.raw`^first,?\s+(?:a|one|the)\s+question(?:\s+for\s+you)?${SIR}${END}`,
  "i",
);

// stripInterviewPreamble(reply) -> reply with a leading announcement of the
// interview itself removed, or reply unchanged if it does not open with
// exactly one of the two shapes above. A reply that was only the
// announcement comes back as "", which the caller's existing !question
// branch already treats as nothing to speak.
export function stripInterviewPreamble(reply) {
  if (typeof reply !== "string") return "";
  const match = BEFORE_I_INTERVIEW.exec(reply) || FIRST_A_QUESTION.exec(reply);
