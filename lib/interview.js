...
// it. So a start is never proposed until every facet has been read back --
// "so: the flaky builder test, in jarvis, touching only the test file, done
// when npm test passes twice - have I got that right?" -- and answered. The
// tag carries that in two more keys beside have=: confirming= names the
// facets the question being asked reads back, confirmed= the ones Krane has
// said yes to. readyToPropose is the check, and server.js refuses to propose
// a start that fails it, speaking a read-back composed by readBack from the
// model's own brief instead of the proposal. The one exception is Krane
// saying to just go (wantsToProceed): that is him overriding the rule out
// loud, and proceed already outranks coverage.
...
// readyToPropose(state, now) -> whether a start may be proposed off this
// interview.
//
...
// yes it just heard, and the natural tag to report it with is the start tag
// itself. Requiring an interview tag with confirmed= in between would cost
...
export function interviewBlock(state, now = Date.now()) {
  ...
  // Only for a start: tell and interrupt keep the older rule, where covered
  // is enough, because the thing at the far end of them already has the
  // context a brief exists to supply. unconfirmed here is what has been
  // neither confirmed nor read back; a facet being read back is waiting on
  // Krane's answer, which is the next thing the model will hear.
  const unconfirmed = verb === "start" ? unconfirmedFacets(state) : [];
  ...
  if (proceed) {
    ...
    tail =
      `Every facet is ${verb === "start" ? "confirmed" : "covered"}: propose now, unless an answer left something genuinely open - ` +
...
// readBack({ task, repo, brief }, facets) -> the question server.js speaks
// when a start tag arrives with facets nobody has confirmed, or "" when
...
// forbids.
export function readBack({ task, repo, brief } = {}, facets = FACETS) {
  ...
  const where = clean(parsed.where, MAX_CLAUSE_CHARS) || clean(repo, 40);
  ...
  for (const facet of wanted) {
    if (facet === "goal") {
      if (goal) clauses.push(`the goal is ${withoutStop(goal)}`);
      else assumptions.push("I do not have a goal for it at all");
    } else if (facet === "where") {
      if (where) clauses.push(`in ${withoutStop(where)}`);
