...
export function wantsToProceed(text) {
  ...
  // a negation here -- it is the escape phrase itself.
  if (/\b(don't|dont|do not|not|never|wait|hold on|stop)\b/.test(normalized)) return false;
...
export function composeBrief({ task, brief, notes } = {}) {
  ...
  // Clean each note and drop empties. This is the text a real session receives
  // as its prompt, and a pure function that is safe only when its caller
  // sanitised first is not safe.
  const usable = Array.isArray(notes)
