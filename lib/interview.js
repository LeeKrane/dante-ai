...
export function interviewBlock(state, now = Date.now()) {
  ...
  // not a competing one.
  //
  // spokenFor is set only by the machine's own synthetic read-back tag
  // (noteInterview, from tag.spokenFor === true), never by the model, and
  // withdrawConfirming -- called when that read-back comes back a no or a
  // correction -- keeps it. So spokenFor true with nothing left in
  // `confirming` can only mean one thing: the machine's read-back was just
  // withdrawn, and what Krane says next is the correction, not a fresh
  // answer to a question the model itself never asked.
  let tail;
  if (proceed) {
  ...
  } else if (spokenFor && confirming.length === 0) {
