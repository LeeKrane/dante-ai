...
//
// When the read-back was the machine's, the answer is read by the machine
// too (server.js's answerHeld, through lib/confirm.js's readAnswer): a yes
// lifts the hold on exactly the tag that was read back, a no or a longer
// correction hands the facets back to the model as unconfirmed
// (withdrawConfirming). The model's own read-backs are answered in the
// model's own context, and the tag it writes next is where it reports the
// yes -- that half is prompt-enforced, and readyToPropose's comment says why.
