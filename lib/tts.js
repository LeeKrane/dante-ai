...
// byte arrives at 350 ms. A caller that awaits the complete body only saves
// the ~250 ms the whole clip finishes earlier; the rest is what a streaming
// client can spend, and speakStream has nothing to spend unless the bytes are
// already in flight.
export function buildTtsRequest(text, cfg) {
  ...
  // 0 is Fish's own default, so it is left out rather than sent explicitly.
  if (Number.isFinite(cfg.volume) && cfg.volume !== 0) body.prosody = { ...body.prosody, volume: cfg.volume };
  // cfg.pitch is deliberately never read here. Fish's prosody object has no
  // pitch field -- only speed, volume, normalize_loudness -- and an unknown key
  // is not worth the risk of sending. Pitch is applied client-side instead, by
  // resampling the clip in public/app.js (see public/pitch-policy.js).
  return {
    url: TTS_URL,
    headers: {
...
//
// The same request buildTtsRequest describes, read as it arrives instead of
// after the whole body. Paired
...
// impure half of a module (opts.bin, opts.root, opts.settings) -- this has a
// loop worth testing, so the fetch call it drives needs the same seam.
