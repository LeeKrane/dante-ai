...
// A caller-supplied duration, or the default when it is missing, negative or
// not a number at all: every timeout and poll interval in this file is read
// the same way, and reading them the same way in one place is what keeps a
// zero or a NaN from ever becoming "wait forever".
...
export function tellSession(spec = {}, opts = {}) {
  ...
  return new Promise((resolvePromise) => {
    ...
    }, positiveMs(opts.timeoutMs, TELL_TIMEOUT_MS));
    deadline.unref?.();
...
// killed outright -- the same grace listAgents gives its listing. The client,
// not the session: see stopViaDaemon.
...
export async function stopSession(record = {}, opts = {}) {
  ...
  if (record?.kind === "background") {
    ...
    const asked = await stopViaDaemon(id, {
      ...
      killGraceMs: positiveMs(opts.killGraceMs, KILL_GRACE_MS),
    });
...
function stopViaDaemon(id, { bin, timeoutMs, killGraceMs }) {
  return new Promise((resolvePromise) => {
    let child;
    try {
      child = spawn(bin ?? "claude", ["stop", id], {
        // stdin closed rather than inherited, for the reason lib/builder.js
        // documents: a CLI holding the parent's stdin waits for input that is
        // never coming.
        stdio: ["ignore", "pipe", "pipe"],
    ...
    // Drained and discarded: unread pipes fill and the child blocks. stderr is
    // capped, and clean() below collapses it to one line of at most 200
    // characters, which is as much of a refusal as is worth saying out loud.
    child.stdout.on("data", () => {});
    child.stderr.on("data", (chunk) => {
      if (stderr.length < 500) stderr += chunk;
    });
