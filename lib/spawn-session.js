...
import { KILL_GRACE_MS, runCli } from "./run-cli.js";
...
export async function tellSession(spec = {}, opts = {}) {
  ...
  if (code !== 0) return { ok: false, error: clean(stderr, 200) || `that session refused, exit ${code}` };
  try {
...
async function stopViaDaemon(id, { bin, timeoutMs, killGraceMs }) {
  ...
  return { ok: false, error: clean(stderr, 200) || `the CLI exited ${code}` };
}
