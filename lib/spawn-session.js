...
import { MAX_BRIEF_CHARS } from "./interview.js";
...
export function buildStartArgs(spec = {}) {
  ...
  // brief, it is the prompt and the task is only the label.
  const brief = clean(spec.brief, MAX_BRIEF_CHARS);
...
export function buildTellArgs(spec = {}) {
  ...
  // deliver half an instruction.
  const text = clean(spec.text, MAX_BRIEF_CHARS);
