...
import { processAlive } from "./session-files.js";
...
//
// `spec` is { name, sessionId, task, systemPrompt?, model?, effort? }.
export function buildStartArgs(spec = {}) {
  ...
  // The task goes after the options terminator, always. Without it a task that
  // happens to start with a dash — or one a model wrote to look like a flag —
  // would be read as an option rather than as the thing to do.
  args.push("--", task);
...
// from the roster rather than from a local tally, and it is `ownRunning` doing
// the counting (lib/agents.js): only a session Dante itself started counts
// against the ceiling, one that died does not, and a hand-started terminal
// session is on the roster without ever weighing on the five.
...
export function buildTellArgs(spec = {}) {
  const sessionId = typeof spec.sessionId === "string" ? spec.sessionId : "";
  const text = clean(spec.text, MAX_TASK_CHARS);
...
//
// `record` is a roster entry, so its pid came from one of two places Dante
// never asked about out loud: `claude agents --json`, or -- for a session a
// person started by hand -- a filename under ~/.claude/sessions that
// lib/session-files.js turned into a roster row. Never from anything spoken.
// `opts.kill` is the seam the tests use.
export async function stopSession(record = {}, opts = {}) {
  ...
  // A file-sourced record's procStart closes a TOCTOU the CLI-sourced ones
  // never had to worry about: the roster this record came from can be
  // several seconds old by the time a stop actually runs, and a stop that
  // waited behind a spoken approval can be the better part of a minute
  // behind that. If the original pid exited and the OS handed the number to
  // something else in that window, a bare SIGTERM here would hit a stranger's
  // process rather than the session anyone approved stopping. Records with
  // no procStart -- every CLI-sourced one, and every record in the tests
  // above this one -- skip the check exactly as before.
  if (typeof record.procStart === "string" && !(await processAlive(pid, record.procStart))) {
    return { ok: false, error: "that session's process is no longer the one I saw" };
  }
