...
// How long the `claude stop` client gets after a polite SIGTERM before it is
// killed outright. The client, not the session: see stopViaDaemon.
...
//
// Anything else -- an interactive session in somebody's terminal, or a
// background record the CLI listed without an id to stop it by -- has no
// lease to settle, and SIGTERM is the right ask. SIGTERM, and only ever
...
export async function stopSession(record = {}, opts = {}) {
  ...
  if (id) {
    ...
    const asked = await stopViaDaemon(id, opts);
    ...
    return { ...(await waitGone(gone, opts)), via: "daemon" };
  }
  ...
  return { ...(await waitGone(gone, opts)), via: "signal" };
}

// The daemon's own short id for a background session, or null when the record
// is not one the daemon can be asked about. Everything the roster carries came
// from the CLI, but the id still has to look like one before it becomes an
// argument: a value beginning with "-" would be read as an option, and a
// listing is not a place to take that on trust.
function daemonId(record) {
  if (record?.kind !== "background") return null;
  const id = typeof record.id === "string" ? record.id.trim() : "";
  return /^[A-Za-z0-9][\w.-]{0,99}$/.test(id) ? id : null;
}
...
// misleading.
async function waitGone(gone, opts) {
...
// stopViaDaemon(id, opts) -> Promise<{ ok, error? }>. Never rejects.
//
...
// it is the one that names what went wrong.
function stopViaDaemon(id, opts) {
  return new Promise((resolvePromise) => {
    let child;
    try {
      child = spawn(opts.bin ?? "claude", ["stop", id], {
        // stdin closed rather than inherited, for the reason lib/builder.js
        // documents: a CLI holding the parent's stdin waits for input that is
        // never coming.
        stdio: ["ignore", "pipe", "pipe"],
    ...
    const timeoutMs = Number.isFinite(opts.timeoutMs) && opts.timeoutMs > 0 ? opts.timeoutMs : STOP_TIMEOUT_MS;
    const deadline = setTimeout(() => {
      // The CLI here is only a client of the daemon, not the session, so
      // ending it is safe: the worst case is a stop that was not delivered,
      ...
      }, Number.isFinite(opts.killGraceMs) && opts.killGraceMs > 0 ? opts.killGraceMs : STOP_KILL_GRACE_MS);
      hardKill.unref?.();
      finish({ ok: false, error: "the CLI did not answer" });
    }, timeoutMs);
    deadline.unref?.();
