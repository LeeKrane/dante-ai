...
export async function stopSession(record = {}, opts = {}) {
  ...
  const remaining = () => ({ ...opts, timeoutMs: Math.max(1, deadline - Date.now()) });
  ...
  if (record?.kind === "background") {
    ...
    const asked = await stopViaDaemon(id, remaining());
    ...
    return { ...(await waitGone(gone, remaining())), via: "daemon" };
  }
  ...
  return { ...(await waitGone(gone, remaining())), via: "signal" };
}
