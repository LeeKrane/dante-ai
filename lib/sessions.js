...
//
// `<alias>-<n>-<task-slug>`, so jarvis-1-builder-test-fix. The alias says which
// repository, the number is per-repository (which is what makes "jarvis one"
// and "fitness one" both sayable), and the slug is what tells two sessions in
...
export function buildName(spec = {}, taken = []) {
  const alias = slugify(spec.alias, 1) || "session";
  const number = Number.isInteger(spec.number) && spec.number > 0 ? spec.number : 1;
  // A kind's own hint beats the task text: "review" is a better name for a
  // review than the first four words of whatever was said about it.
  const slug = slugify(spec.hint) || slugify(spec.task);

  const base = `${alias}-${number}`;
  const wanted = (slug ? `${base}-${slug}` : base).slice(0, MAX_NAME_CHARS);

  const used = new Set((Array.isArray(taken) ? taken : []).filter((name) => typeof name === "string"));
  if (!used.has(wanted)) return wanted;

  // A number collides when a session was started outside jarvis and happens to
  // share the name, or when the counter was reset. Suffixing beats reusing:
