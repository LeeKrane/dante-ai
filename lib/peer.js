...
// A steered instruction is a sentence or two, authored by a model or spoken
// through one, that is about to be written into somebody else's live session.
// The cap bounds how much of somebody's turn one message can consume, in the
// same size class as lib/spawn-session.js's MAX_TASK_CHARS and
// lib/slack.js's MAX_TEXT_CHARS.
...
// Same class lib/spawn-session.js and lib/agents.js strip, redeclared here for
// the same reason both of those redeclare it rather than import one another's:
// this text is composed by a model, or spoken through one, and is about to
// become the literal input of somebody else's live session. Control
// characters could forge structure there; bidi overrides could make it read
// one way on screen and another to the process reading it.
const UNPRINTABLE = /[\u0000-\u001f\u007f-\u009f\u200e\u200f\u202a-\u202e\u2066-\u2069]/g;

// Whitespace is collapsed BEFORE the unprintables are stripped, and the order
// is the whole point — copied from lib/spawn-session.js's clean() along with
// the reason: a newline is both, and stripping it first fuses the words on
// either side into one ("fix the teststhen push"). Collapsing first turns it
// into the space it was standing in for, and the strip then takes what's left.
...
export function buildMessageFrame(spec = {}) {
  ...
  const text = clean(spec.text, MAX_MESSAGE_CHARS);
  if (!text) return null;
...
export function steerText(text) {
  const cleaned = clean(text, MAX_MESSAGE_CHARS);
  if (!cleaned) return "";
...
export function planDelivery(verb, text) {
  ...
  const content = interrupt ? steerText(text) : clean(text, MAX_MESSAGE_CHARS);
...
export async function readPeerAddress(pid, opts = {}) {
  ...
  // deliberately does not use it: sendToSession checks sessionId instead. For
  // a CLI-sourced record that sessionId comes from the very roster listing
  // (parseRoster) that produced the pid in the first place, so the two are
  // already guaranteed to name the same session by construction — procStart
  // would only be re-deriving a check the caller already gets for free.
  //
  // For a file-sourced record (lib/session-files.js) that guarantee does not
  // hold the same way: its sessionId comes from this very state file,
  // which readPeerAddress is re-reading right here, so checking it against
  // itself is circular rather than independent. It is still safe, for a
  // different reason: a pid recycled since that record was built and now
  // owned by a new Claude session would have overwritten this file with that
  // session's own sessionId, which the mismatch above still catches, and a
  // pid recycled by anything else has no vetted socket to answer on, so the
  // connect below simply fails.
  const features =
