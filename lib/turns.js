...
// across a restart, the same reason the roster rides along.
export function mergeTurns(texts, opts = {}) {
  ...
  const roster = rosterBlock(opts?.roster, opts?.recalled, opts?.aliases ?? {}, opts?.now);
  // Under the roster lines rather than in a block of its own: to the model both
  // are the same kind of thing, facts about the machine and the conversation
  // that are true right now, and one block is one thing to read past.
  const interview = typeof opts?.interview === "string" ? opts.interview.trim() : "";
  const block = [roster, interview].filter(Boolean).join("\n");
  return block ? `${block}\n\n${said}` : said;
}
