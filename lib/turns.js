...
function rosterBlock(roster, recalled, aliases, now, interview, watching) {
  const interviewLine = typeof interview === "string" ? interview.trim() : "";
  const watchLine = watchingLine(watching);

  if (!Array.isArray(roster)) {
    // No roster, but the watching and interview lines are still machine
    // state worth carrying, in that order, same as when a roster is present.
    const lines = [watchLine, interviewLine].filter(Boolean);
...
export function mergeTurns(texts, opts = {}) {
  ...
  const block = rosterBlock(
    opts?.roster, opts?.recalled, opts?.aliases ?? {}, opts?.now, opts?.interview, opts?.watching,
  );
