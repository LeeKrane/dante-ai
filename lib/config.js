...
export function loadFishConfig(path = DEFAULT_PATH) {
  ...
  return {
    ...
    // code path for it, but this loader dropped the field on the floor -- only
    // the tests ever exercised that branch. It has never once reached Fish.
    volume: Number.isFinite(cfg.volume) ? cfg.volume : 0,
  };
}

const SUPABASE_PATH = join(homedir(), ".config", "jarvis", "supabase.json");
