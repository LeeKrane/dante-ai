...
//
// What Dante SAID about a session, once read, is a different thing and is kept:
// dispatchRead in server.js files it as a note in lib/notes.js, so the
// conversation that follows can build on it and a restart does not forget what
// was said. A deleted session is still unreadable -- the rule above is
// untouched -- but its note survives until the notes directory prunes it on its
// own schedule. Nothing in this module, or in lib/transcript.js, ever consults
// a note to answer a read; a note is written from what was said, never read
// from to decide what to say.
//
// That is why the list below is a filter over things that already exist -- the
// live roster, the store's record of what Dante started, the files on disk --
// and never a list this module maintains.
//
// The whitelist applies here too. A repository dropped from memory must stop
// being readable, or "Dante only sees the repositories you named out loud" is
// true of live sessions and quietly false of finished ones.

// `within` is the whitelist rule itself: a session's cwd counts as inside a
// workspace when it equals the root or sits underneath it, never on a plain
// startsWith (that would put /home/me/jarvis-notes inside /home/me/jarvis).
// `aliasFor` is alias -> path inverted, so a session's directory can be named
// by the workspace's alias instead of the directory's own basename -- a
// finished session run from a worktree (Dante's own start:verb sessions call
// EnterWorktree and move under <repo>/.claude/worktrees/<name>, and the
// whitelist below already treats that as inside the repo) must be named that
// way or describeFinished reads it back as "repo-persistence: jarvis-10-...",
// the same bug label() in lib/agents.js had. When two aliased workspaces
// nest, the longest matching root wins. Both rules live once, in
// lib/agents.js, and are imported here rather than copied.
import { aliasFor, within } from "./agents.js";
...
//
// `remembered` is the store's own record of what Dante started (getSessions in
...
// `null` means the listing failed and Dante genuinely does not know. A read is
...
export function recallableSessions(remembered, roster, opts = {}) {
  ...
  // that would report every session Dante started today as finished.
  ...
  for (const [sessionId, entry] of Object.entries(known)) {
    ...
    // An undated record is kept: it is a real session Dante started, and
  ...
  // Sessions Dante did not start but can see: a terminal in a whitelisted
...
export function describeFinished(records, aliases = {}, now = Date.now()) {
  ...
  const parts = shown.map((record) => {
    ...
    // A session Dante named already starts with its repo alias
