...
// loadNote is only called for the handful this actually returns -- the same
// two-pass shape server.js would otherwise have to write itself every time it
// wants to fold notes into a turn. A topic listNotes reports but loadNote then
// fails to reload (deleted between the two calls, or corrupted past its own
// header) is skipped rather than turned into a gap in the result.
export function recentNotes(dir, n = MAX_CONTEXT_NOTES) {
  ...
  for (const entry of entries) {
    const note = loadNote(dir, entry.topic);
