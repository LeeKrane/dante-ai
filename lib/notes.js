...
export function notesContext(notes, now = Date.now()) {
  ...
  const blocks = sorted.map((note) => {
    ...
    const tail = cleanBody(body.slice(-MAX_CONTEXT_CHARS_PER_NOTE), MAX_CONTEXT_CHARS_PER_NOTE);
...
// resolves in lib/confirm.js: { name, sessionId, cwd, task, running,
// stoppedAt? }. The session id's first eight characters ride along in the
// topic (not the bare name) because two different sessions commonly reuse one
...
export function sessionNoteSpec(record, question, text, now) {
  ...
  // running is a fact settled by the live roster, so it wins outright; a
  // record with no roster to check falls back to whether it was ever stopped
  // from here, and anything left over is a session that simply finished on
  // its own.
  const status = record.running === true ? "running" : Number.isFinite(record.stoppedAt) ? "stopped" : "finished";
  const rawFacts = { subject: record.name, status };
