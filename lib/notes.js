...
// normalizeSubject: the same reduction lib/agents.js's normalizeName applies
// to a spoken session name (lowercase, non-alphanumerics collapsed to one
// dash, leading/trailing dashes trimmed), redeclared here for the same
// reason cleanLine is -- this module must not import lib/agents.js, and
// notes.js's own subject fact needs the identical normalization to ever
// line up with a name lib/agents.js matched a person's words against.
...
// foldNotes(tracker, dir, now?, hint?) -> { context, flag }. Loads the
// recent notes for `dir` (pinning whichever one `hint` names, per
...
// and returns both the machine-state block to fold into this turn's prompt
// and the spoken sentence for whatever contradiction that touch surfaced
// against a note touched earlier in the conversation -- `tracker.pending()`,
// not `tracker.settle()`: this only COMPUTES the flag, it never marks it
