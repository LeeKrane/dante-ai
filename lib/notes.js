...
export function mergeSection(note, section) {
  ...
  const key = readSectionKey(section?.text);
...
export function notesContext(notes, now = Date.now()) {
  ...
  const blocks = sorted.map((note) => {
    ...
    // (either may be absent), in the order they actually happened -- a
    // discussion only ever follows a read (topicIsLive gates recordDiscussion
    // on a live read topic), so chronological order also puts the read
    // first for budget below. Each is clipped from its HEAD, not its tail: a
    ...
    const picked = [newestOfKind("read"), newestOfKind("discussion")]
      .filter(Boolean)
      .sort((a, b) => (a.at ?? 0) - (b.at ?? 0));

    // One shared budget, spent in the order above, so a short read leaves
    // room for the discussion and a long one does not starve it entirely.
    let budget = MAX_CONTEXT_CHARS_PER_NOTE;
    const parts = [];
    for (const section of picked) {
      const clipped = section.text.slice(0, budget);
...
// recentNotes(dir, n?) -> the `n` most recently updated notes, fully loaded.
// listNotes supplies the ordering cheaply (it reads only headers), and
// loadNoteFile is only called for the handful this actually returns -- the
// same two-pass shape server.js would otherwise have to write itself every
// time it wants to fold notes into a turn. Loaded from entry.path, not
// re-derived from entry.topic via loadNote/notePath, for the same reason
// pruneNotes reads entry.path directly: a hand-named file's basename does
// not always survive topicSlug, and re-deriving the path here would silently
// load nothing, or the wrong file, for exactly the notes pruneNotes would
// also mis-target. A topic listNotes reports but loadNoteFile then fails to
// reload (deleted between the two calls, or corrupted past its own header)
// is skipped rather than turned into a gap in the result.
export function recentNotes(dir, n = MAX_CONTEXT_NOTES) {
  const count = Number.isInteger(n) && n > 0 ? n : MAX_CONTEXT_NOTES;
  const entries = [...listNotes(dir)].sort((a, b) => (b.updated ?? 0) - (a.updated ?? 0)).slice(0, count);
...
// foldNotes(tracker, dir, now?) -> { context, flag }. Loads the recent notes
// for `dir`, touches every one of them into `tracker` (folding a note into
// the prompt counts as accessing it, the same as a read does), and returns
// both the machine-state block to fold into this turn's prompt and the
...
// will actually speak it. Impure (recentNotes reads `dir`), never throws.
export function foldNotes(tracker, dir, now = Date.now()) {
  const notes = recentNotes(dir);
  for (const note of notes) tracker.touch(note);
  return {
    context: notesContext(notes, now),
