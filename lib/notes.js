...
// whitespace collapsed, trailing punctuation stripped, re-trimmed since
// stripping trailing punctuation can expose a trailing space the first trim
// never saw -- "what is it doing ?" and "what is it doing?" must key the
// same), prefixed with "q:" -- or the bare constant "read" when the first
// line carries no question at all (no "Asked: " prefix, or one whose
// question is empty once normalized). A read with no question is "read it
// back, plain" -- there is only one of those worth keeping at a time, so
// the newest supersedes the last. A read with a question is one answer per
// distinct question, so two different questions about the same session
// both get to keep their section. The "q:" prefix on a real question is
// what keeps a question that itself normalizes to the word "read" (e.g.
// "Asked: Read?") from colliding with the bare, question-less sentinel --
// they are two different keys on purpose.
...
export function notesContext(notes, now = Date.now()) {
  ...
  const blocks = sorted.map((note) => {
    ...
    // body. Each is clipped from its HEAD, not its tail: a spoken read
    // opens with the question and the answer and trails off into
    // supporting detail, the opposite of a transcript tail, and a
    // discussion opens with what was actually asked.
    ...
    // Every OTHER "read" section, newest first: mergeSection keeps one
    // section per distinct question on disk, and without this only ever
    // the single newest read would reach a prompt -- a second question's
    // answer would sit in the file forever, never folded, the moment a
    // newer question got asked after it.
    const olderReads = sections
      .filter((s) => s.kind === "read" && s !== read)
      .sort((a, b) => (b.at ?? 0) - (a.at ?? 0));

    // Budget spent in a fixed priority order -- newest read, newest
    // discussion, then older reads newest first -- not whichever order the
    // sections happen to fall in chronologically. A topic can be read more
    // than once while its window stays live (read, discussion, a second
    // read), and when it has, the newest read can be the MORE recent of
    // the two even though it is chronologically after a discussion sitting
    // between the two reads; spending budget in chronological order would
    // then burn it on the stale discussion first and starve the fresh read
    // of the very thing this note exists to keep. The read is the answer;
    // the discussion is commentary on it. A read at or past the whole
    // budget is meant to starve everything after it entirely -- that is
    // the intended priority, not a bug to route around.
    let budget = MAX_CONTEXT_CHARS_PER_NOTE;
    const parts = [];
    const spend = (section) => {
      if (!section) return;
      const clip = section.text.slice(0, Math.max(budget, 0));
      if (!clip) return;
      parts.push(clip);
      // Charge the "\n" this clip will be joined to whatever comes after it
      // by, not just the clip's own length -- otherwise the running budget
      // handed to the NEXT section is one char too generous per join, the
      // combined body can land MAX_CONTEXT_CHARS_PER_NOTE+1 (or more, with
      // several joins) chars long, and cleanBody's own final slice would
      // then silently drop characters off the very end -- not necessarily
      // from the part that ran over, once there is a third part waiting
      // for whatever budget the first two left it.
      budget -= clip.length + 1;
    };
    spend(read);
    spend(discussion);
    for (const older of olderReads) spend(older);

    const body = cleanBody(parts.join("\n"), MAX_CONTEXT_CHARS_PER_NOTE);
...
// limits) -> { note, pruned } or null when the topic slug is empty, the
// merge added nothing, or the save failed. The convenience the server
// calls: loads the existing note (or starts a new one, with `created`
// pinned to `at`), merges facts with new values winning, refreshes
// summary/about only when the caller actually supplied one, folds in the
// section via mergeSection (one read section per distinct question, rather
// than one per call), and saves. A note's title is set once, at creation,
// and never rewritten by a later call -- only summary/about are meant to be
// refreshed as a topic's story develops.
export function writeSection(dir, topic, input, limits) {
  ...
  // mergeSection returns the exact object it was given, by reference, when a
  // "read" section's text cleaned to nothing -- an empty read has nothing to
  // record. Saving anyway would rewrite the note's facts/summary/about under
  // `merged`'s stale `updated` (whatever it was before this call), which is
  // worse than no save at all: it forfeits saveNote's own "the newest note
  // is never pruned" exemption for a save that, by that same `updated`, is
  // NOT the newest thing in the directory -- exactly the note that was just
  // written can be the one pruneNotes deletes on its own next run.
  if (withSection === merged) return null;
