#!/usr/bin/env node
// Claude Code hook -> jarvis. Reads one hook event on stdin, posts it to the
// local jarvis server, exits.
//
// You install this yourself; jarvis never writes ~/.claude/. Its own build deny
...
//     Code, and "jarvis is not running" is not a thing a session should be
//     told about, let alone blocked by.
//   - It prints nothing. Hook stdout can be fed back into the session.
//   - It gives up fast. A jarvis that is down or wedged must cost a session
...
process.stdin.on("end", () => {
  ...
  // Only the fields jarvis reads. The transcript path, the tool input and
  ...
  req.on("error", done); // jarvis not running is the ordinary case, not an error
  req.end(body);
});
