...
changing every second is not worth a message. The one exception is a finished session: the CLI's
listing carries no end time, so the roster poller keeps the tick it first sees a session `done`
(`trackEnded` in `lib/agents.js`, keyed off the raw listing so a one-tick filter miss cannot move
it; a Stop hook can refresh it, and the memory store carries it across a restart), stamps it on
the roster as `endedAt`, the wire carries it, and the row's clock stops there instead of counting
on from `startedAt`. It lives as a corner overlay, up from load and closed with `s` -- the
